...
=============================================================
  ...
  Backend : Flask + Alibaba Qwen3-ASR 1.7B (CPU, local)
  Frontend: HTML/JS (record button → JSON output)
  Usage   : python app.py → open http://localhost:5000

SETUP:
  pip install flask torch qwen-asr
  No API key needed — runs fully locally on CPU.
  First run downloads the Qwen3-ASR 1.7B weights (~3-4GB).
=============================================================
"""
...
# ── Load Qwen3-ASR model once at startup ──────────────────
print("=" * 55)
print("  AMBIENT NOTES  |  Qwen3-ASR 1.7B  |  Local CPU Mode")
print("=" * 55)
print("  Loading Qwen3-ASR 1.7B (first run downloads weights)...")
...
<head>
  ...
  <title>Ambient Notes | Qwen3-ASR Local STT</title>
  <style>
...
<body>
  ...
  <div class="header">
    ...
    <div class="model-pill"><span class="dot"></span> Qwen3-ASR 1.7B · running locally on CPU</div>
  </div>
  ...
  <div class="card">
    <div class="section-label">Transcript <span class="badge">Qwen3-ASR 1.7B</span></div>
...
<script>
  ...
  function stopRecording() {
    ...
    setStatus('Transcribing locally with Qwen3-ASR… (CPU, may take a moment)', 'processing');
  }
...
def transcribe():
    ...
    try:
        ...
        # language=None activates automatic language identification (52 languages)
        ...
        # Qwen3-ASR result objects may expose duration/language depending on version
        ...
        response = {
            ...
            "model":     "Qwen3-ASR-1.7B (local CPU)"
        }
