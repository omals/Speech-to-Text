...
=============================================================
  ...
  Backend : Flask + OpenAI Whisper (local, base model)
...
# "base" is a good balance of speed and accuracy (~140MB download on first run)
print("=" * 55)
print("  AMBIENT NOTES  |  Whisper base  |  Local Mode")
print("=" * 55)
print("  Loading Whisper base model...")
print("  (First run downloads ~140MB — please wait)\n")
...
<head>
  ...
  <title>Ambient Notes | Whisper Local STT</title>
  <style>
...
<body>
  ...
  <div class="header">
    ...
    <div class="model-pill"><span class="dot"></span> Whisper base · running locally</div>
  </div>
  ...
  <!-- Output card -->
  <div class="card">
    <div class="section-label">Transcript <span class="badge">Whisper base</span></div>
...
<script>
  ...
  function stopRecording() {
    ...
    setStatus('Transcribing locally with Whisper…', 'processing');
  }
  ...
  function showTiming(data, totalMs) {
    ...
    document.getElementById('tInfer').textContent = data.timing
      ? fmt(data.timing.inference_ms) : '--';
    document.getElementById('tAudio').textContent = data.audio_duration
      ? data.audio_duration.toFixed(1) + 's' : '--';
    document.getElementById('tRTF').textContent = data.timing && data.timing.rtf
      ? data.timing.rtf + '×' : '--';
  }
...
def transcribe():
    ...
    # Save uploaded audio to a temp file (ffmpeg handles webm → wav internally)
    ...
    try:
        ...
        result = model.transcribe(
            ...
            fp16=False,           # CPU-safe (set True if running on CUDA GPU)
            verbose=False
        )
